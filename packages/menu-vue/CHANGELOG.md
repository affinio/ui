...
- ## Summary
  ...
  Expose menu positioning controls in the public Vue API.
  - Added `placement`, `align`, `gutter`, and `viewportPadding` props on `UiMenu` and `UiSubMenu`.
  - Added the same props on `UiMenuContent` and `UiSubMenuContent` as local overrides.
  - Forwarded positioning props to `useMenuPositioning` so behavior stays consistent across projects.

  ## User impact

  Integrators can configure menu placement/alignment/offsets without internal hacks while preserving existing defaults.

  ## Migration
  - No migration required.
  - Existing menus keep current defaults.
  - Optional: pass new props on root/content components when custom positioning is needed.
