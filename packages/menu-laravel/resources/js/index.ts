...
import type { MenuOverlayTraits, MenuState } from "@affino/menu-core"
import type { SurfaceReason } from "@affino/surface-core"
import { getDocumentOverlayManager, type OverlayKind } from "@affino/overlay-kernel"
...
type PointerIntent = "pointer" | "keyboard"
...
const MENU_ROOT_SELECTOR = "[data-affino-menu-root]"
...
function hydrateScope(scope: ParentNode): void {
...
function bindLivewireHooks(): void {
  ...
  const target: EventTarget | null = typeof document !== "undefined" ? document : window
  target?.addEventListener("livewire:navigating", () => mutationObserver?.disconnect())
...
function mutationTouchesMenu(mutation: MutationRecord): boolean {
  ...
  return nodes.some((node) => {
    ...
    if (node.matches?.(MENU_ROOT_SELECTOR)) {
      return true
    }
