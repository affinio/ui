...
- Geometry edits: `resizeEntities` updates nodes, shapes, and texts, and scales node-relative ports with resized nodes; `insertEdgeWaypoint`, `moveEdgeWaypoint`, and `removeEdgeWaypoint` cover baseline polyline/orthogonal edge editing. `rotateEntities` and `alignEntities` are core commands because they share history, locks, diagnostics, geometry invalidation, and serialization rules; Vue should expose toolbar controls that dispatch those commands.
