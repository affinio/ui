...
- Resize gestures: adapters can call `createDiagramInteractionController(...).beginResizeHandle(id, handle, event)` for `nw`, `ne`, `se`, and `sw` handles. Core maps pointer deltas through entity rotation, anchors the opposite rotated corner, exposes one transient `resizePreview` in the interaction snapshot, leaves the persisted scene unchanged during drag, then commits one `resizeEntities` history entry on pointer up.
