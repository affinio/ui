Цель
  ...
  - Editor must-have slice: implemented in core and the Vue demo with clipboard import/export/duplicate, id remapping, z-order/layer commands, resize commands, anchored rotation-aware resize-handle previews, waypoint edits, keyboard helpers, additive/toggle/containment selection modes, lock constraints, fit helpers, capability checks, query/search primitives, and diagnostics/perf counters.
