Цель
  ...
  - Slices 1-7: implemented in `packages/diagram-core` as a headless package with normalized scene state, geometry cache, uniform-grid spatial indexes, command/history engine, interaction controller, snapping APIs, and internal services for scene state/snapshots, history, geometry, spatial indexing, viewport fit math, and clipboard subgraph import/export.
