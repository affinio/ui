...
- Added source-owned preorder, preorder-index, depth, and subtree-end indexes during registration.
- Canonical expanded normalization now sorts valid expanded ids by preorder index instead of scanning the full preorder, dropping expand/collapse traversal reads to zero in the benchmark harness.
- Remaining Slice 2 work: expose depth/preorder/subtree metadata through the future windowed row metadata API.
...
- Valid node additions, including parent/child additions in the same patch, now update affected parent/root lists plus source indexes without full source finalization.
- Existing-node reparent patches with valid non-cyclic parents now rebuild only affected parent child lists plus source indexes instead of full source finalization.
- Mixed add+reparent patches still route through full source finalization.
...
- Wired `/treeview` in `packages/demo-vue` to exercise a virtualized ~2.4k-node fixture, search projection, direct match styling, DOM scroll synchronization, rendered-window counters, and input-focus preservation in the sandbox route.
