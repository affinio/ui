...
2026-09-19: аудит, baseline diagram core 25 tests и Vue 15 tests, builds passed, direct Node ESM import обоих dist успешен на Node 24. Stress 1k/5k/10k/50k выявил лишнюю сортировку уже упорядоченного full-scan query и повторное построение render order; устранены full-scan sort и revision-scoped cached ordered ids. На 50k после оптимизаций warm query: visible около 4ms, entity search около 13–17ms. Закрыты G01/G02/G04: replacement publishes old+new IDs включая empty scene, input entities owned at boundary, history state committed before publication, locked text и limit=0 guards. G03/G09 продвинуты: unique gesture history keys, cancel resets tool, stale frame callbacks ignored, coalesced history patches flatten steps вместо nested closure chains; long gesture regression покрыт 1000 updates, bounded history policy остаётся отдельным решением. G05 продвинут: readonly snapshot entity values и неизменённые maps переиспользуются по identity через WeakMap/changed-id sharing; добавлен regression на unchanged entity/map sharing, но persistent-map и retention benchmark остаются открытыми. G06 продвинут: dependency index локализует node→port→edge invalidation и перестраивается только при topology/entity membership changes; основной node move больше не сканирует все edges/ports, focused regression сохранился. G07 продвинут: UniformGridIndex ограничивает cell expansion и делает bounded full-entry fallback для huge/invalid rect. G08 продвинут: Vue visible projection использует core geometry cache, stable render entity wrappers для неизменной geometry и больше не делает duplicate initial refresh. G11 начат: benchmark и package docs используют публичное поле id для waypoint commands; waypoint benchmark проверяет changed результат каждого edit/undo. Остальные acceptance gates остаются открыты: persistent snapshot maps, browser/perf/CI.
