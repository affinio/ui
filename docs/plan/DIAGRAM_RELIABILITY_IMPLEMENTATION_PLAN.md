...
2026-09-19: аудит, baseline diagram core 25 tests и Vue 15 tests, builds passed, direct Node ESM import обоих dist успешен на Node 24. Закрыты G01/G02/G04: replacement publishes old+new IDs включая empty scene, input entities owned at boundary, history state committed before publication, locked text и limit=0 guards. G03/G09 частично harden: unique gesture history keys, cancel resets tool, stale frame callbacks ignored. G07 начат: UniformGridIndex ограничивает cell expansion и делает bounded full-entry fallback для huge/invalid rect. G08 продвинут: Vue visible projection использует core geometry cache, stable render entity wrappers для неизменной geometry и больше не делает duplicate initial refresh. Остальные acceptance gates остаются открыты: bounded history budget, incremental indexes, rendering order, browser/perf/CI.
