...
2026-09-19: аудит, baseline diagram core 25 tests и Vue 15 tests, builds passed, direct Node ESM import обоих dist успешен на Node 24. Stress 1k/5k/10k/50k выявил лишнюю сортировку уже упорядоченного full-scan query и повторное построение render order; устранены full-scan sort и revision-scoped cached ordered ids. Persistent readonly overlay maps устранили full map copy при single-entity publication. На 50k smoke после оптимизаций: visible query около 4–12ms, entity search около 13–17ms, drag/undo около 8ms, memory около 135MB; это benchmark harness signal, не универсальный SLA. Закрыты G01/G02/G04: replacement publishes old+new IDs включая empty scene, input entities owned at boundary, history state committed before publication, locked text и limit=0 guards. G03/G09 продвинуты: unique gesture history keys, cancel resets tool, stale frame callbacks ignored, coalesced history patches flatten steps вместо nested closure chains; long gesture regression покрыт 1000 updates, bounded history policy остаётся отдельным решением. G05 продвинут: readonly snapshot entity values, неизменённые maps и changed entity overlays переиспользуются по identity; добавлен regression на unchanged entity/map sharing. G06 продвинут: dependency index локализует node→port→edge invalidation и перестраивается только при topology/entity membership changes; основной node move больше не сканирует все edges/ports, focused regression сохранился. G07 продвинут: UniformGridIndex ограничивает cell expansion и делает bounded full-entry fallback для huge/invalid rect. G08 продвинут: Vue visible projection использует core geometry cache, stable render entity wrappers для неизменной geometry и больше не делает duplicate initial refresh. G11 начат: benchmark и package docs используют публичное поле id для waypoint commands; waypoint benchmark проверяет changed результат каждого edit/undo. Остальные acceptance gates остаются открыты: retention/browser/perf/CI calibration.
...
2026-09-19: G12 browser gate подготовлен: `scripts/smoke-components-demo.mjs` проверяет demo diagram selection, rotation/revision и zoom вместе с dialog lifecycle; CI visual job запускает его после Chromium install. Локальная arm64 Ubuntu26.04 среда не поддерживает Playwright Chromium, поэтому runtime evidence ожидается от GitHub runner.
...
2026-09-19: G06 продвинут: добавлен deterministic mutation-sequence oracle после move/resize/waypoint/delete/undo; indexed visibility и nearest-port результаты совпали с brute-force oracle во всех состояниях. Diagram-core test 32/32 прошёл. Spatial indexes теперь обновляют только invalidated geometry и удаляют исчезнувшие IDs; полная rebuild остаётся только для cold/dirty initialization.
