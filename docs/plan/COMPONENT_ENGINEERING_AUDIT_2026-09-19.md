...
- Diagram: G01–G05 are closed; G06–G12 remain in progress. Structural sharing/retention, bounded `history.maxEntries`, dependency indexing, incremental spatial index updates for invalidated geometry (including stale-ID removal), query benchmarks with cold/warm separation, isolated Node/Vite SSR consumer import, demo build, and CI dialog/diagram smoke are covered. Browser execution and remaining renderer/index retention evidence remain open.
