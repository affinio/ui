# Repository Docs

This directory contains architecture, integration, process, and operations docs for Affino UI primitives and adapters.

Start with:

- [Main index](./index.md)
- [Monorepo scripts runbook](./monorepo-scripts-runbook.md)
- [Docs lifecycle policy](./docs-lifecycle-policy.md)
- [Archive index](./archive/README.md)
- [Architecture reference](./reference/architecture.md)
- [Diagram packages](./reference/diagram-packages.md)
- [Process and governance](./process/README.md)
- [Operations runbooks](./ops/slo-sla.md)
- [ADR index](./adr/README.md)
